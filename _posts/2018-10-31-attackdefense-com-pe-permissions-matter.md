...
Funnily enough, I had access to the ```shadow``` file, but I saw no password hash inside it.
I decided to check whether I had write access to the ```shadow``` or ```passwd``` file, so I could place a password hash for the root user (some legacy systems accept password hashes in ```passwd```).  
